        ...
        uchar3 * outPixels)
{
    ...
    { 
		int i = r * width + c;
		float x = 0, y = 0, z = 0;
		for (int filter_r = 0; filter_r < filterWidth; filter_r++) //Filter rows
		{
			for (int filter_c = 0; filter_c < filterWidth; filter_c++) //Filter cols
			{
				int filter_idx = filter_r * filterWidth + filter_c;
				int in_R, in_C;
				
				if ((r - filterWidth/2) + filter_r < 0) 
					in_R = 0;
				else if ((r - filterWidth/2) + filter_r > height - 1) 
					in_R = height - 1;
				else 
					in_R = (r - filterWidth/2) + filter_r;
				
				if ((c - filterWidth/2) + filter_c < 0) 
					in_C = 0;
				else if ((c - filterWidth/2) + filter_c > width - 1) 
					in_C = width - 1;
				else 
					in_C = (c - filterWidth/2) + filter_c;
					
				int image_idx = in_R * width + in_C;
				x += filter[filter_idx] * inPixels[image_idx].x;
				y += filter[filter_idx] * inPixels[image_idx].y;
				z += filter[filter_idx] * inPixels[image_idx].z;
			}
		}
		uchar3 outPix = make_uchar3(x,y,z);
		outPixels[i] = outPix;
    }	
}
    ...
    { 
		...
		for (int filter_r = 0; filter_r < filterWidth; filter_r++) //Filter rows
		{
			for (int filter_c = 0; filter_c < filterWidth; filter_c++) //Filter cols
			{
				int filter_idx = filter_r * filterWidth + filter_c;
				int in_R, in_C;
				in_R = threadIdx.y + filter_r;
				in_C = threadIdx.x + filter_c;	
				int image_idx = in_R * s_width + in_C;
				x += filter[filter_idx] * s_inPixels[image_idx].x;
				y += filter[filter_idx] * s_inPixels[image_idx].y;
				z += filter[filter_idx] * s_inPixels[image_idx].z;
			}
		}
		uchar3 outPix = make_uchar3(x,y,z);
		outPixels[i] = outPix;
    ...
    { 
		...
		for (int filter_r = 0; filter_r < filterWidth; filter_r++) //Filter rows
		{
			for (int filter_c = 0; filter_c < filterWidth; filter_c++) //Filter cols
			{
				...
				in_C = threadIdx.x + filter_c;	
				int image_idx = in_R * s_width + in_C;
				x += dc_filter[filter_idx] * s_inPixels[image_idx].x;
				y += dc_filter[filter_idx] * s_inPixels[image_idx].y;
				z += dc_filter[filter_idx] * s_inPixels[image_idx].z;
			}
		}
		uchar3 outPix = make_uchar3(x,y,z);
		outPixels[i] = outPix;
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		// Call kernel
		size_t nBytes = (blockSize.x + filterWidth - 1) * (blockSize.y + filterWidth - 1) * sizeof(uchar3);
		...
		else if (kernelType == 2)
		{
			...
			blurImgKernel2<<<gridSize, blockSize, nBytes>>>(d_inPixels, width, height, d_filter, filterWidth, d_outPixels);
		}
		else
		{
			// TODO: call blurImgKernel3
			blurImgKernel3<<<gridSize, blockSize, nBytes>>>(d_inPixels, width, height, filterWidth, d_outPixels);
		}
		timer.Stop();
