		...
		uchar3 * outPixels)
{
	...
	if(r < height && c < width)
	{
		...
		for(int filter_row = 0; filter_row < filterWidth; filter_row++)
		{
			for(int filter_col = 0; filter_col < filterWidth; filter_col++)
			{
				int row_pixel = r - radius + filter_row;
				int col_pixel = c - radius + filter_col;

				if(row_pixel < 0)
				{
					row_pixel = 0;
				}
				else if(row_pixel > height - 1)
				{
					row_pixel = height - 1;
				}
				...
				else if(col_pixel > width - 1)
				{
					col_pixel = width - 1;
				}

				int filter_idx = filter_row * filterWidth + filter_col;
				int pixel_input = row_pixel * width + col_pixel;

				red += inPixels[pixel_input].x * filter[filter_idx];
				green += inPixels[pixel_input].y * filter[filter_idx];
				blue += inPixels[pixel_input].z * filter[filter_idx];
			}
		}
		outPixels[i].x = red;
		outPixels[i].y = green;
		...
		bool useDevice=false, dim3 blockSize=dim3(1, 1))
{
	...
	else // Use device
	{
		...
		size_t filter_nBytes = filterWidth * filterWidth * sizeof(float);

		CHECK(cudaMalloc(&d_inPixels, nBytes));
		CHECK(cudaMalloc(&d_outPixels, nBytes));
		CHECK(cudaMalloc(&d_filter, filter_nBytes));

		CHECK(cudaMemcpy(d_inPixels, inPixels, nBytes, cudaMemcpyHostToDevice));
		CHECK(cudaMemcpy(d_filter, filter, filter_nBytes, cudaMemcpyHostToDevice));
